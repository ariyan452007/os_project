 ...
 * Wraps ProcessBuilder, translating our Command objects into actual OS-level
 * child processes.
 * Configures the file descriptor mapping directly at the OS level using
    ...
    public static int execute(Command cmd, InputStream in, PrintStream out, PrintStream err) {
        ...
        try {
            ...
            if (cmd.stdoutRedirect != null) {
                pb.redirectOutput(cmd.stdoutAppend ? ProcessBuilder.Redirect.appendTo(new File(cmd.stdoutRedirect))
            ...
            if (cmd.stderrRedirect != null) {
                pb.redirectError(cmd.stderrAppend ? ProcessBuilder.Redirect.appendTo(new File(cmd.stderrRedirect))
            ...
            if (cmd.isBackground) {
                int jobId = JobTable.addJob(p, String.join(" ", cmd.args) + " &");
